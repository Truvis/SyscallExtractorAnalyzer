...
def syscall_extractor(file: str):
    ...
    if file.endswith(".LOG"):
        ...
        FIN = common_elements(SysCallsInFile, SysCall64List)
        FoundInList.append(FIN)
...
if __name__ == "__main__":
    ...
    if VERBOSE == "y": sm.p_msg('===', 'notice', '1', 'y')
    with multiprocessing.Pool(int(Threads)) as p:
        p.map(syscall_extractor, [os.path.join(FilePath, file) for file in os.listdir(FilePath)])
    print("\r")
    if VERBOSE == "y": sm.p_msg('Finding Common Syscalls for 64 Bit', 'notice', '1', 'y')
    # Find common elements between all the nested lists
    nFoundInList = list(set.intersection(*map(set, FoundInList)))
    sm.p_msg('SysCalls in Common [Linux 64 Bit]:  ' + ' , '.join(map(str, nFoundInList)), 'ok', '2', 'y')
